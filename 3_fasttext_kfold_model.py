...
class fasttextModel(object):
    def split_train_test(self, datasetFile, testing_size=0.2):
    ...
    def fasttext_kfold_model(self, df, k, lrs, epochs, dims, loss_fns, ngrams):
        ...
        for train_index, test_index in kf.split(df['label'], df['text']):
            ...
            '''
            self.write_kfold_best_results(best_results)
            ...
            best_model.save_model("./data/kfold_train_test_data/best_kfold" +
                              str(best_results["kfold_counter"])+"_model.bin")
    ...
    def write_kfold_best_results(self, kfold_results):
        with open('./data/kfold_train_test_data/kfold_best_results.csv', 'a') as results:
            ...
            write.writerow([kfold_results["conf"], kfold_results["f_score"],
                       kfold_results["p_score"], kfold_results["r_score"], kfold_results["kfold_counter"]])

if __name__ == '__main__':
    fasttext_model_object = fasttextModel()
    # split the data set into training and validating sets
    fasttext_model_object.split_train_test(source_dataset)
